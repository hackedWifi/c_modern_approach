 ...
 *
 *    Description:  Modify the inventory.c of Section 16.3 by adding a price member to 
 *                  the part structure. The insert function should ask the user for the
 *                  price of a new item. The search and print functions should display
 *                  the price. Add a new command tat allows the user to change the price
 *                  of a part.
 *
 *                  
 *        Version:  1.0
 ...
 struct part inventory[MAX_PARTS];
  ...
  for ( ; ; ) {
    printf ( "Enter operation code i (insert), s (search)," 
        "u (update), p (print), c (change price) q (quit) " );
    scanf ( " %c", &code );
